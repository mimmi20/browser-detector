...
final readonly class UseragentDeviceCode implements DeviceCodeInterface
{
    ...
    public function getDeviceCode(string $value): string | null
    {
        ...
        $regexes = [
            '/^mozilla\/[\d.]+ \((?:andr[o0]id|tizen) [\d.]+(?:[^;]+)?(?:; arm(?:_64)?|; x86|; x64|; harmonyos|; mobile)?; (?P<devicecode>[^;\/]+)(?:(?:\/[^ ]+)? +(?:build|hmscore|release))[^)]+\)/i',
            ...
            '/dalvik\/[\d.]+ \(linux; andr[o0]id v?[\d.]+(?:[^;]+)?; (?P<devicecode>[^);\/]+)(?:[);\/]?[^);\/]* +(?:build|hmscore|release|miui)[^)]+)\)/i',
            '/dalvik\/[\d.]+ \(linux; andr[o0]id v?[\d.]+(?:[^;]+)?; (?P<devicecode>[^);\/]+)(?:[);\/]?[^);\/]+)?\)/i',
            '/dalvik\/[\d.]+ \(linux; andr[o0]id v?[\d.]+\/viber [\d.]+ ; (?P<devicecode>[^);\/]+)[su]p1a/i',
            ...
            '/ucweb\/[\d.]+ \(linux; android [\d.]+; (?P<devicecode>[^);\/]+)(?:[^)]+)?\)/i',
            ...
            '/mozilla\/[\d.]+ \(compatible; msie [\d.]+; windows phone(?: os)? [\d.]+; trident\/[\d.]+; iemobile\/[\d.]+; alcatel; (?P<devicecode>[^);\/]+)(?:[);\/]?[^);\/]*)\)/i',
            '/mozilla\/[\d.]+ \(java; (?P<devicecode>[^);\/]+)(?:[);\/]?[^);\/]*)\)/i',
            '/mozilla\/[\d.]+ \(linux; ?[\d.]+; ?(?P<devicecode>[^;\/]+)(?:[^)]+)?\)? applewebkit/i',

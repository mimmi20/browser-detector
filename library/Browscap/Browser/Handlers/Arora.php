...
class Arora extends BrowserHandler
{
    ...
    public function canHandle($userAgent)
    {
        if (!$this->utils->checkIfStartsWith($userAgent, 'Mozilla/')
            && !$this->utils->checkIfStartsWith($userAgent, 'Arora/')
        ) {
            return false;
        }
        
        if (!$this->utils->checkIfContainsAll($userAgent, array('AppleWebKit', 'Arora'))) {
            return false;
        }
    ...
    {
        $doMatch = preg_match('/Arora\/([\d\.]+)/', $userAgent, $matches);

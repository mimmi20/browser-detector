...
class CssMin
{
    ...
    public static function minify($css, $options = 'remove-last-semicolon')
    {
        ...
        if (in_array('preserve-urls', $options)) {
            ...
            $css = preg_replace_callback('/url\s*\((.*)\)/siU', array(self, '_cssminEncodeUrl'), $css);
        }
    ...
    public static function toArray($css, $options = '')
    {
        ...
        if (count($items[0]) > 0) {
            for ($i = 0; $i < $c = count($items[0]); $i++) {
                ...
                $r[] = array(
                    'keys'   => self::_cssminArrayClean($keys),
                    'styles' => self::_cssminArrayClean($styles)
                );
            }
        }
     ...
     * @return array
     */
    private static function _cssminArrayClean(array $array)
    {
        $r = array();
        
        if (self::_cssminArrayIsAssoc($array)) {
     ...
     * @return boolean
     */
    private static function _cssminArrayIsAssoc(array $array)
    {
        if (!is_array($array)) {
            return false;
        } else {
     ...
     * @return string
     */
    private static function _cssminEncodeUrl(array $match)
    {
     ...
     * @return string
     */
    private static function _cssminDecodeUrl(array $match)
    {

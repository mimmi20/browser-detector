 ...
 * @copyright 2007-2010 Unister GmbH
 */
class Institutes extends ModelAbstract
{
    /**
     * Table name
     *
     ...
     * @var String
     */
    protected $_primary = 'idInstitutes';
    ...
    public function getId($value)
    {
        ...
        if (!isset(self::$_prepared['getid_' . $postfix])) {
            ...
            $select->from(
                array('i' => $this->_name),
                array('id' => 'idInstitutes')
            );
            if (is_numeric($value)) {
                $select->where('i.idInstitutes = :name');
            } else {
                $select->where('LOWER(i.codename) = :name');
            }
    ...
    public function find($instituteId = null)
    {
        ...
        if (!isset(self::$_prepared['find'])) {
            ...
            $select->where('`i`.`idInstitutes` = :id');

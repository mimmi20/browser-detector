 ...
 * @copyright 2007-2010 Unister GmbH
 */
class PartnerSites extends ModelAbstract
{
    /**
     * Table name
     *
     ...
     * @var String
     */
    protected $_primary = 'idPartnerSites';
    ...
    public function loadByCampaign($campaign = '')
    {
        ...
        if (!is_object($campaign) || null === $campaign->idPartnerSites) {
            return null;
        }

        $portalId = $campaign->idPartnerSites;
    ...
    public function getId($paid)
    {
        ...
        if (!isset(self::$_prepared['getid_' . $postfix])) {
            ...
            $select->from(
                array('p' => $this->_name),
                array('id' => 'idPartnerSites')
            );

            if (is_numeric($paid)) {
                $select->where('p.idPartnerSites = :paid');
            } else {
                $select->where('p.name = :paid');

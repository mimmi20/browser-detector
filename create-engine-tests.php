...
foreach (new \RecursiveIteratorIterator($iterator) as $file) {
    ...
    if (!preg_match('/class (.*) extends Engine/', $filecontent, $matches)
        && !preg_match('/class (.*)\\n    extends Engine/', $filecontent, $matches)
    ) {
    ...
    $testGenerator = new \SebastianBergmann\PHPUnit\SkeletonGenerator\TestGenerator($namespace . '\\' . $className, $fullpath, $className . 'Test', 'tests\\Detector\\Engine\\' . $className . 'Test.php');
    $testContent = $testGenerator->generate();

    $testContent = str_replace('<?php', '<?php' . "\n\nnamespace BrowserDetectorTest\\Detector\\Engine;\n\nuse BrowserDetector\\Detector\\Engine\\$className;\n\n", $testContent);
